...
def test_create_pr_step_success(mock_emit, mock_subprocess, mock_which) -> None:
    ...
    # Step calls: rev-parse (branch), gh pr list, rev-parse (base), rev-list (delta),
...
def test_create_pr_step_gh_command_failure(mock_subprocess, mock_emit, mock_which) -> None:
    ...
    # push (success), pr create (failure)
    mock_rev_parse = Mock(returncode=0, stdout="my-branch\n", stderr="")
    mock_pr_list = Mock(returncode=0, stdout="[]", stderr="")
    mock_base_branch = Mock(returncode=0, stdout="origin/main\n", stderr="")
    mock_delta = Mock(returncode=0, stdout="1\n", stderr="")
...
def test_create_pr_step_multi_repo_success(mock_emit, mock_subprocess, mock_which) -> None:
    ...
    # Two repos: each needs rev-parse, pr list (empty), rev-parse (base), rev-list (delta),
    ...
    mock_base_a = Mock(returncode=0, stdout="origin/main\n", stderr="")
    mock_delta_a = Mock(returncode=0, stdout="1\n", stderr="")
    ...
    mock_base_b = Mock(returncode=0, stdout="origin/main\n", stderr="")
    mock_delta_b = Mock(returncode=0, stdout="1\n", stderr="")
...
def test_create_pr_step_multi_repo_failure(mock_emit, mock_subprocess, mock_which) -> None:
    ...
    # Two repos: each needs rev-parse, pr list (empty), rev-parse (base), rev-list (delta),
    # push, pr create (fail) — 6 calls each = 12 total
    mock_rev_parse_a = Mock(returncode=0, stdout="my-branch\n", stderr="")
    mock_pr_list_a = Mock(returncode=0, stdout="[]", stderr="")
    mock_base_a = Mock(returncode=0, stdout="origin/main\n", stderr="")
    mock_delta_a = Mock(returncode=0, stdout="1\n", stderr="")
...
def test_create_gitlab_mr_step_success(mock_emit, mock_subprocess) -> None:
    ...
    # Step calls: rev-parse (branch), mr list (empty), rev-parse (base), rev-list (delta),
    # git push, glab mr create
    mock_rev_parse = Mock(returncode=0, stdout="my-branch\n", stderr="")
...
def test_create_gitlab_mr_step_glab_command_failure(
    ...
    # Step calls: rev-parse, mr list (empty), rev-parse (base), rev-list (delta),
...
def test_create_gitlab_mr_step_glab_not_found(mock_subprocess, mock_get_logger, mock_emit) -> None:
    ...
    # Step calls: git rev-parse, glab mr list (FileNotFoundError propagates to outer handler)
    # FileNotFoundError is NOT caught by _try_adopt_existing, so it propagates up
    mock_rev_parse = Mock(returncode=0, stdout="my-branch\n", stderr="")

    mock_subprocess.side_effect = [
        mock_rev_parse,
        FileNotFoundError("glab not found"),  # glab mr list — propagates to outer handler
    ]
