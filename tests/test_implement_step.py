...
def mock_load_required_artifact(mock_context) -> WorkflowContext:
    ...
    def _load(key, artifact_cls=None) -> object:
        value = mock_context.data.get(key)
        if value is None:
            raise StepInputError(f"Required artifact '{key}' not found")
        return value
...
class TestImplementPlanStepRun:
    ...
    @patch("rouge.core.workflow.steps.implement_step.emit_comment_from_payload")
    @patch.object(ImplementPlanStep, "_implement_plan")
    def test_run_saves_data_to_context(
        self,
        mock__implement_plan,
        mock_emit,
        mock_load_required_artifact,
        ...
        """Test that implementation data is saved to context.data."""
        mock_context = mock_load_required_artifact
        mock_context.data = {"plan_data": sample_plan_data}
        ...
        assert result.success is True
        assert mock_context.data["implement_data"] == sample_implement_data
        assert mock_context.data["implement"] == sample_implement_data
...
class TestImplementDirectStepRun:
    ...
    def test_run_fails_when_no_issue(self, direct_mock_context) -> None:
        """Test that run fails when context.issue is None."""
        direct_mock_context.issue = None
    ...
    def test_run_saves_data_to_context(
        ...
        """Test that implementation data is saved to context.data on success."""
        issue_mock = Mock()
        issue_mock.description = "Implement feature X directly"
        direct_mock_context.issue = issue_mock
        ...
        assert result.success is True
        assert direct_mock_context.data["implement_data"] == sample_data
        assert direct_mock_context.data["implement"] == sample_data
