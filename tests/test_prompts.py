...
def _make_fake_package(files: dict[str, str]):
...
class TestPromptId:
    ...
    def test_values_have_no_adw_prefix(self):
...
class TestParseTemplate:
    ...
    def test_no_front_matter_returns_full_text_as_body(self):
        text = "# My Prompt\n\nDo something."
    ...
    def test_extracts_model_sonnet(self):
        text = "---\nmodel: sonnet\n---\n\nBody."
        _, model, _ = _parse_template(text, PromptId.CLASSIFY)
        assert model == "sonnet"

    def test_extracts_model_opus(self):
        text = "---\nmodel: opus\n---\n\nBody."
        _, model, _ = _parse_template(text, PromptId.CLASSIFY)
        assert model == "opus"

    def test_invalid_model_is_ignored(self):
        text = "---\nmodel: gpt-4\n---\n\nBody."
        _, model, _ = _parse_template(text, PromptId.CLASSIFY)
        assert model is None

    def test_invalid_model_logs_warning(self, caplog):
        import logging
        text = "---\nmodel: gpt-4\n---\n\nBody."
    ...
    def test_thinking_key_is_silently_dropped(self):
    ...
    def test_both_description_and_model_extracted(self):
    ...
    def test_unclosed_front_matter_returns_full_text(self):
...
class TestPromptRegistryGet:
    ...
    def test_get_returns_prompt_template(self):
    ...
    def test_get_missing_template_raises_file_not_found(self):
        registry = PromptRegistry()
    ...
    def test_template_body_is_non_empty(self):
        registry = PromptRegistry()
        template = registry.get(PromptId.CLASSIFY)
        assert template.body.strip() != ""

    def test_template_body_has_no_front_matter_markers(self):
    ...
    def test_classify_template_has_model_sonnet(self):
        """classify.md front matter declares model: sonnet."""
        registry = PromptRegistry()
        template = registry.get(PromptId.CLASSIFY)
        assert template.model == "sonnet"

    def test_templates_without_model_return_none(self):
    ...
    def test_no_template_contains_thinking_key(self):
...
class TestPromptRegistryValidate:
    ...
    def test_validate_succeeds_for_all_packaged_templates(self):
        """validate() loads all PromptIds without raising."""
        registry = PromptRegistry()
        registry.validate()  # must not raise

    def test_validate_populates_entire_cache(self):
    ...
    def test_validate_raises_if_template_missing(self):
    ...
    def test_validate_raises_if_template_body_empty(self):
...
class TestPromptRegistryRender:
    ...
    def test_render_multiple_args_joined_with_newline(self):
        registry = PromptRegistry()
        result = registry.render(PromptId.CLASSIFY, ["line one", "line two"])
        assert "line one\nline two" in result.text

    def test_render_empty_args_with_arguments_placeholder(self):
        """$ARGUMENTS replaced with empty string when args=[]."""
        registry = PromptRegistry()
        result = registry.render(PromptId.CLASSIFY, [])
        assert "$ARGUMENTS" not in result.text

    def test_render_no_arguments_placeholder_appends_args(self):
    ...
    def test_render_no_arguments_placeholder_empty_args_no_append(self):
    ...
    def test_render_returns_model_from_template(self):
        registry = PromptRegistry()
        result = registry.render(PromptId.CLASSIFY, ["x"])
        assert result.model == "sonnet"

    def test_render_returns_none_model_when_not_in_front_matter(self):
        registry = PromptRegistry()
        result = registry.render(PromptId.FEATURE_PLAN, ["x"])
        assert result.model is None

    def test_render_only_first_arguments_replaced(self):
...
class TestPromptIdTemplateContract:
    ...
    def test_template_file_exists(self, prompt_id: PromptId):
    ...
    @pytest.mark.parametrize("prompt_id", list(PromptId))
    def test_template_loads_via_registry(self, prompt_id: PromptId):
    ...
    @pytest.mark.parametrize("prompt_id", list(PromptId))
    def test_template_renders_without_error(self, prompt_id: PromptId):
...
class TestModuleLevelHelpers:
    ...
    def test_get_registry_returns_prompt_registry(self):
    ...
    def test_get_registry_eagerly_validates_all_templates(self):
    ...
    def test_render_prompt_returns_rendered_prompt(self):
        result = render_prompt(PromptId.CLASSIFY, ["test input"])
        assert isinstance(result, RenderedPrompt)
        assert "test input" in result.text

    def test_render_prompt_uses_shared_registry(self):
