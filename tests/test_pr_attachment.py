...
def _make_context(
    ...
    """Build a mock WorkflowContext with optional issue and plan data.

    The mock's ``load_optional_artifact`` delegates to ``context.data.get(key)``,
    matching the real WorkflowContext behavior.
    """
    ctx = MagicMock()
    ctx.data = {}
    ctx.issue = issue
    ...
    if plan_data is not None:
        ctx.data["plan_data"] = plan_data

    def _load_optional(key, artifact_cls=None):
...
class TestLoadAndRenderAttachment:
    """Integration tests for load_and_render_attachment (data loading + rendering)."""

    def test_plan_data_model_in_cache(self) -> None:
        """PlanData model in context data is rendered correctly."""
    ...
    def test_both_issue_and_plan_in_context(self) -> None:
        """Both issue and PlanData model produce full output."""
        from rouge.core.models import Issue

        issue = Issue(id=1, description="The spec text")
        plan = PlanData(plan="The plan body", summary="Short summary")
        ctx = _make_context(issue=issue, plan_data=plan)
    ...
    def test_only_issue_from_context(self) -> None:
        """Output contains spec but no plan when only issue is available."""
        from rouge.core.models import Issue

        issue = Issue(id=1, description="Spec from issue")
        ctx = _make_context(issue=issue)
    ...
    def test_only_plan_data_no_issue(self) -> None:
        """Output contains plan but no spec when only plan_data is in context."""
    ...
    def test_neither_present(self) -> None:
        """Returns None when neither issue nor plan data is present."""
        ctx = _make_context()
