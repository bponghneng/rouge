...
- Stores result in context.data['plan_data']
...
def context_with_issue(issue: Issue) -> WorkflowContext:
    """Create a workflow context with context.issue set (as FetchIssueStep would)."""
    ctx = WorkflowContext(
        issue_id=42,
...
class TestClaudeCodePlanStepHappyPath:
    ...
    def test_happy_path(
        ...
        """Agent succeeds, plan data stored in context."""
        plan_data = PlanData(
...
class TestClaudeCodePlanStepFailureCases:
    ...
    def test_agent_failure(
        ...
        """Step fails when agent returns failure."""
