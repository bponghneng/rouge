...
class TestLoadRequiredArtifact:
    ...
    def test_raises_step_input_error_when_key_not_found(self) -> None:
        """Test load_required_artifact raises StepInputError when key is not in data."""
        ctx = WorkflowContext(adw_id="adw-req-missing")

        with pytest.raises(StepInputError, match="Required data 'plan_data' not found"):
            ctx.load_required_artifact("plan_data")

    def test_returns_value_when_key_exists(self) -> None:
        """Test load_required_artifact returns the value when key exists in data."""
        ...
        result = ctx.load_required_artifact("plan_data")
    ...
    def test_returns_cached_value_from_context_data(self) -> None:
        """Test load_required_artifact returns existing value from context data."""
...
class TestLoadOptionalArtifact:
    ...
    def test_returns_none_when_key_not_found(self) -> None:
        """Test load_optional_artifact returns None when key is not in data."""
        ctx = WorkflowContext(adw_id="adw-opt-missing")

        result = ctx.load_optional_artifact("plan_data")

        assert result is None

    def test_does_not_raise_when_key_not_found(self) -> None:
        """Test load_optional_artifact does not raise an exception on missing key."""
    ...
    def test_returns_value_when_key_exists(self) -> None:
        """Test load_optional_artifact returns the value when key exists in data."""
    ...
    def test_returns_cached_value_from_context_data(self) -> None:
        """Test load_optional_artifact returns existing value from context data."""
        cached_value = PlanData(plan="Cached plan", summary="Cached summary")
        ctx = WorkflowContext(
