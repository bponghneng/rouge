...
def get_agent(provider: Optional[str] = None) -> CodingAgent:
    ...
    2. ROUGE_AGENT_PROVIDER environment variable
    3. Default to "claude"
    ...
    if provider is None:
        provider = os.getenv("ROUGE_AGENT_PROVIDER")
    ...
    if provider not in _AGENTS:
        ...
        raise ValueError(
            f"Agent provider '{provider}' not registered. Available providers: {available}"
        )
...
def get_implement_provider() -> str:
    ...
    """
    # Check ROUGE_IMPLEMENT_PROVIDER first (most specific)
    provider = os.getenv("ROUGE_IMPLEMENT_PROVIDER")

    # Fallback to ROUGE_AGENT_PROVIDER
    if provider is None:
        provider = os.getenv("ROUGE_AGENT_PROVIDER")
