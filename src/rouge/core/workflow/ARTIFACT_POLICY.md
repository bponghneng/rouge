...
4. **Ordering-only dependencies are never read as artifacts.** A step whose registry entry marks a dependency as `ordering-only` (e.g. `code-quality` on `implement`, `compose-request` on `acceptance`) must not call any artifact-loading helper for that dependency. The dependency exists only to guarantee correct DAG sequencing.
