...
class WorkflowContext:
    ...
    def get_step_data(self, key: str) -> Any:
        """Return ``self.data[key]``, raising :class:`StepInputError` if missing.

        Args:
            key: The context data key to look up.

        Returns:
            The value stored under *key*.

        Raises:
            StepInputError: If *key* is not present in ``self.data``.
        """
        try:
            return self.data[key]
        except KeyError:
            raise StepInputError(
                f"Required step data '{key}' not found. "
                "Ensure the preceding step completed successfully and stored its data."
            )

    def get_optional_step_data(self, key: str) -> Optional[Any]:
