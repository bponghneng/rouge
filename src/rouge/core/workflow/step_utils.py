...
def load_and_render_attachment(context: "WorkflowContext") -> str | None:
    ...
    review-context comment body. Returns ``None`` when neither source is present.

    Args:
        context: The workflow context providing data loading.

    Returns:
        Rendered Markdown string, or ``None`` if both sources are absent.
    """
    # Get issue description from context.issue (set by FetchIssueStep/FetchPatchStep)
    spec_text = None
    if context.issue and context.issue.description:
        spec_text = context.issue.description

    # Get plan data from context.data
    plan_data = context.load_optional_artifact("plan_data")
