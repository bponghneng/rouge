...
Usage:
    python -m cape-worker --worker-id <worker_id> [--poll-interval <seconds>] [--log-level <level>]

Example:
    python -m cape-worker --worker-id alleycat-1 --poll-interval 10 --log-level INFO
"""
...
from cape.core.utils import make_adw_id
...
class IssueWorker:
    ...
    def setup_logging(self) -> logging.Logger:
        ...
        """
        logger = logging.getLogger(f"cape_worker_{self.config.worker_id}")
    ...
    def execute_workflow(self, issue_id: int, description: str) -> bool:
        """
        Execute the cape-adw workflow for the given issue.
        ...
        try:
            ...
            # Note: Not capturing output allows real-time logging from cape-adw
            app_root = Path(os.environ.get("CAPE_APP_ROOT", Path.cwd()))
