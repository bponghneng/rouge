"""
Migration 017: Add 'direct' workflow/issue type.

Widens the schema so 'direct' is a valid issue type alongside 'full', 'patch',
and 'thin'.

Changes:
  1. Replace the CHECK constraint on issues.type to allow
     ('full', 'patch', 'thin', 'direct').
  2. Recreate get_and_lock_next_issue RPC to filter on
     ('full', 'patch', 'thin', 'direct').

Rollback reverts both changes to ('full', 'patch', 'thin') only.
"""
...
step(
    ...
    ALTER TABLE issues ADD CONSTRAINT issues_type_check
        CHECK (type IN ('full', 'patch', 'thin', 'direct'));
    """,
    """
    ALTER TABLE issues DROP CONSTRAINT IF EXISTS issues_type_check;
    ALTER TABLE issues ADD CONSTRAINT issues_type_check
...
step(
    ...
    BEGIN
        ...
        WITH next_issue AS (
            SELECT i.id
            FROM issues i
            WHERE i.type IN ('full', 'patch', 'thin', 'direct')
