...
Examine the repo `~/git/rouge/rouge/` and follow `Instructions` to compose conventional commits from unstaged changes then report the results in the exact format specified `Report`.
...
- Based on the git changes shown above, follow the `Commit Process` to create meaningful conventional commits
